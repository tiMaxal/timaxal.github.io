...
**Expected Behavior:**
- **Template email (with '+')**: user+@gmail.com → Show @eml buttons on domains, NO general message
...
## Band-Aid Fix Scripts (For Reference - DO NOT USE)

These were temporary fix scripts created in the hnsell workspace. They are NOT the solution - fix the source instead:
...
**Note:** These scripts had a bug (corrupted HTML by using `.replace()` on entire content). The real fix must be in hns-portfolio-maker.py source code.
